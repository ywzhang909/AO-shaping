...
class LaserCastEnv(gym.Env):
    ...
    '''
    def __init__(self, max_iter, target_power=None, r_bucket=5, img_size:Tuple[int]=(200,200), history_len:int=10, render_mode='human', img_noise:bool=False) -> None:
        super().__init__()
    ...
    def step(self, action:np.ndarray)->Tuple[dict, float, bool, bool, dict]:
        ...
        if self.img_noise:
            target = _power
            reward = (target - self.history_powers[-1])
            
            if _power>=self.target_power:
                reward = target * self._time_penalty * (self.max_iter - self.iter)
    ...
    def init_env(self) -> None:
        ...
        self.cam.initialize()
        # 获取初始图像
        img = self.cam.get_numpy_image(10)
        
        # 计算图像的质心
        center = np.unravel_index(np.argmax(img), img.shape)[::-1]
        # 重置相机窗口以确保质心在图像中心
        self.img_size, _ = self.cam.reset_window(size=self.img_size, center=center)
