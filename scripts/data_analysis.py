...
# %%
DATA_DIR = Path('/home/tifo/workspace/AO-shaping/data/img2img')
for dir in DATA_DIR.iterdir():
    if dir.is_file():
        if dir.suffix == 'zip':
            ZipFile(dir).extractall(
            dir = dir.with_suffix(''))
        else:
            continue
        
    pkl_list = dir.glob('*.pkl')
    f_cam_dir = dir.joinpath('f_cam')
    f_cam_dir.mkdir(exist_ok=True)
    n_cam_dir = dir.joinpath('n_cam')
    n_cam_dir.mkdir(exist_ok=True)
    wavefront_dir = dir.joinpath('wavefront')
    wavefront_dir.mkdir(exist_ok=True)
    
    all_data_df = []
    for pkl_file in pkl_list:
        data: pd.DataFrame = pd.read_pickle(pkl_file)
        data.columns = [c[1:] if c.startswith('_') else c for c in data.columns]
        data['epoch'] = data['epoch']+1

        for line in tqdm.tqdm(data.itertuples(), total=len(data), desc=f'{pkl_file.absolute()}', dynamic_ncols=True):
            epoch = line.epoch
            np.savez_compressed(f_cam_dir.joinpath(f'{epoch}.npz'), img=line.f_cam)
            np.savez_compressed(n_cam_dir.joinpath(f'{epoch}.npz'), img=line.n_cam)
            np.savez_compressed(wavefront_dir.joinpath(f'{epoch}.npz'), img=line.wavefront)
        data.drop(columns=['f_cam', 'n_cam', 'wavefront'], axis=1, inplace=True)
