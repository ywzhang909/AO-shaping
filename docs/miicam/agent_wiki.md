...
- **单位约定**：Python 驱动层统一使用 **毫秒（ms）**；`put_ExpoTime` 的 SDK 参数为 **微秒（µs）**，驱动在调用处做 `ms * 1000 → µs` 转换。
  - 量程：`0.011 ms ≤ exp ≤ 10000 ms`（即 `11 µs ≤ exp ≤ 10000000 µs`）。
- 曝光-亮度响应（1064nm 激光，8-bit 输出，全局帧统计）：
  - `0.011 ms` → `mean≈22.7`，`max≈252`（已接近 8-bit 上限 255，局部 ROI 可观察更低值）。
  - `0.05~2 ms` → `max` 饱和于 `255`，`mean` 从 `43` 单调增至 `131`。
  - 结论：**mean 可准确反映曝光变化**；若场景较亮，`max` 会先饱和，做质量指标时优先 mean / 亮区包围盒。

### 连续光斑获取（2026-09 实验确认）
- 固定曝光下连续拉流 20 帧，ROI 均值稳定，无丢帧/卡顿。
- 曝光切换后前 1-2 帧可能为旧曝光残留，`get_numpy_image(skip_first=True)` 可规避。
- 若需严格时序，建议在 `reset_exposure_time` 后增加 `time.sleep(0.15~0.3s)` 等待稳定。
