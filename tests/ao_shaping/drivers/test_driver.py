...
def test_all():
    ...
    with NlightDM(keep_when_exit=True) as dm, \
        CameraStreamManager(cam_id=os.environ.get('Far_Cam_ID', 0), exposure_time_ms=800) as cam_in, \
        CameraStreamManager(cam_id=os.environ.get('Near_Cam_ID', 1), exposure_time_ms=80) as cam_out, \
        Thorlab_WFS() as wfs:
