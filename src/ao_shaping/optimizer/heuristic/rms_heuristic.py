"""
元启发式搜索算法优化RMS - 使用RMS作为优化目标
包含粒子群算法(PSO)、Runge-Kutta优化器(RUN)和遗传算法(GA)
"""
import numpy as np
from typing import Literal, Sequence
from ao_shaping.drivers import MlaRes, NlightDM, Thorlab_WFS
from ao_shaping.utils import logger, Recorder
from ao_shaping.optimizer.heuristic.heuristic_algorithms import PSO, RUN, GeneticAlgorithm


def optimizer_rms_heuristic(
    epochs,
    wfs_res: Literal['512', '768'] = '768',
    init_v: Sequence[float | int] = [],
    pupil_center: tuple[float, float] = (0, 0),
    pupil_diameter: float = 2.24,
    algorithm: str = 'PSO',  # 'PSO', 'RUN', 'GA'
    early_stop_threshold: float = 0.12,
) -> Recorder:
    """
    使用元启发式算法优化RMS
    """
    epochs = int(epochs)
    recorder = Recorder(mark='rms_heuristic', mode='min')

    with NlightDM(keep_when_exit=True) as dm:
        if not init_v:
            _init_v = np.zeros(dm.DM_Num, dtype=np.float64)
        else:
            _init_v = np.array(init_v)
        dm.send_voltages(_init_v, 0.5)

        if wfs_res == '768':
            wfs_res_config = MlaRes.Res768
        elif wfs_res == '512':
            wfs_res_config = MlaRes.Res512
        else:
            raise ValueError(f"wfs_res must be '512' or '768', but got {wfs_res}")

        with Thorlab_WFS(wfs_res_config,
                         use_custom_ref=False, high_speed=True,
                         pupil_diameter=pupil_diameter,
                         pupil_center=pupil_center) as wfs:

            def calc_rms():
                wfs.take_image(5)
                wf, statics = wfs.get_wavefront()
                return statics['rms'], wf, statics

            # 获取初始RMS值
            initial_rms, initial_wf, initial_statics = calc_rms()

            # 定义目标函数（最小化RMS）
            def objective_function(voltages):
                # 确保电压在范围内
                clipped_voltages = np.clip(voltages, dm.min_voltage, dm.max_voltage)
                
                # 发送电压
                dm.send_voltages(clipped_voltages)
                
                # 测量RMS
                rms, wf, statics = calc_rms()
                
                return rms  # 返回RMS值（优化器最小化目标函数）

            # 定义搜索边界
            lower_bounds = np.full(dm.DM_Num, dm.min_voltage)
            upper_bounds = np.full(dm.DM_Num, dm.max_voltage)
            bounds = (lower_bounds, upper_bounds)

            # 选择优化算法
            if algorithm == 'PSO':
                optimizer = PSO(
                    dimensions=dm.DM_Num,
                    bounds=bounds,
                    objective_func=objective_function,
                    num_particles=min(30, max(10, dm.DM_Num // 4)),  # 根据DM单元数调整粒子数
                    max_iter=min(epochs // 10, 100),  # 限制最大迭代次数
                    verbose=True
                )
            elif algorithm == 'RUN':
                optimizer = RUN(
                    dimensions=dm.DM_Num,
                    bounds=bounds,
                    objective_func=objective_function,
                    population_size=min(30, max(10, dm.DM_Num // 4)),
                    max_iter=min(epochs // 10, 100),
                    verbose=True
                )
            elif algorithm == 'GA':
                optimizer = GeneticAlgorithm(
                    dimensions=dm.DM_Num,
                    bounds=bounds,
                    objective_func=objective_function,
                    population_size=min(50, max(20, dm.DM_Num // 2)),
                    max_iter=min(epochs // 10, 100),
                    verbose=True
                )
            else:
                raise ValueError(f"Unknown algorithm: {algorithm}")

            # 执行优化
            best_solution, best_fitness = optimizer.optimize()

            # 发送最终电压
            final_voltages = np.clip(best_solution, dm.min_voltage, dm.max_voltage)
            dm.send_voltages(final_voltages)
            
            # 获取最终结果
            final_rms, final_wf, final_statics = calc_rms()

            # 记录初始值
            recorder.append({
                "rms": initial_rms,
                "_v": _init_v,
                "_diff": 0,
                "_gamma": 0,
                "delta": 0,
                "_epoch": 0,
                "_wavefront": initial_wf[np.newaxis, ...],
                "_statics": initial_statics
            })

            # 记录最终结果
            recorder.append({
                "rms": final_rms,
                "_v": final_voltages,
                "_diff": initial_rms - final_rms,
                "_gamma": 0,
                "delta": 0,
                "_epoch": epochs,
                "_wavefront": final_wf[np.newaxis, ...],
                "_statics": final_statics
            })

            logger.info(f"RMS优化完成: 初始RMS={initial_rms:.4f}, 最终RMS={final_rms:.4f}, 改善={initial_rms - final_rms:.4f}")
            
            return recorder