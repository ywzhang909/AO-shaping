...
def optimize_pib(
    ...
    **kwargs,
    ...
    Args:
        ...
        dm (DM | None): DM实例。若为None则创建NlightDM（向后兼容）。
    ...
    with (
        ...
        try:
            ...
            if (
                ...
                shrink_span = np.log(IDEAL_SPOT_RADIUS / r_bucket) / np.log(
                    shrink_ratio
                )
            ...
            def run_adaptive_search(
                ...
                anchor_v = (
                    best_v.copy() if search_anchor == "best" else current_v.copy()
                )
                anchor_objective = (
                    best_objective
                    if search_anchor == "best"
            ...
            with tqdm.tqdm(
                ...
                for epoch in range(1, epochs + 1):
                    disturb_v = (
                        rng.binomial(1, 0.5, (dm.DM_Num,)).astype(float) * 2.0 - 1.0
                    )
                    ...
                    if _should_trigger_adaptive_search(
                        ...
                        search_result = run_adaptive_search(
                            epoch=epoch,
                            current_v=_init_v,
                            current_objective=objective_val,
                        )
                        if search_result is not None:
                            ...
                            if search_result["accepted"]:
                                ...
                                if (
                                    ...
                                    > best_objective
                                    + adaptive_search_state.improvement_tol
                                ):
                                    best_v = _init_v.copy()
