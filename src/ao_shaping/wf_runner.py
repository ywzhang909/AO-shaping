...
from ao_shaping.optimizer.wf.rms import optimizer_rms, optimizer_rms_heuristic
...
def run_spgd_rms(dir, epochs, wfs_res, pupil_diameter, pupil_center, early_stop_threshold, debug, show):
    """波前优化器"""
...
@click.option("-p", "--pupil_diameter", default=2.7, help="瞳孔直径 (default: 2.7)")
@click.option("-c", "--pupil_center", callback=parse_tuple, default="(0,0)", help="瞳孔中心坐标 (default: (0,0))")
@click.option("-a", "--algorithm", type=click.Choice(['PSO', 'RUN', 'GA']), default='PSO', help="选择优化算法 (default: PSO)")
@click.option("-t", "--early_stop_threshold", default=0.0, help="早停阈值 (default: 0.0)")
@click.option("--debug", is_flag=True, help="是否开启调试模式 (default: False)")
@click.option("--show", is_flag=True, help="显示远场光斑CCD图像和优化历史 (default: False)")
def run_heuristic_rms(dir, epochs, wfs_res, pupil_diameter, pupil_center, algorithm, early_stop_threshold, debug, show):
    """使用元启发式算法优化RMS"""
    init_V = [0 for _ in range(64)]
    records = optimizer_rms_heuristic(
        init_v=init_V,
        epochs=epochs,
        wfs_res=wfs_res,
        pupil_diameter=pupil_diameter,
    ...
    if debug:
        save_dir = gen_date_dir(root_dir / f"heuristic_rms_{algorithm}")
        saved_file_name = gen_file_path_uuid(save_dir, 'pkl')
        ...
        plot_funcs["rms_history"](rms_values, ax[0, 0], min_epoch, min_rms)
        # 绘制保存的电压
        plot_funcs["voltages"](min_iter["_v"], ax[0, 1], f"Min RMS: {min_rms:.3f} @ epoch {min_epoch}")
        # 绘制保存的初始波前
        im = plot_funcs["wavefront"](records.first["_wavefront"][0], ax[1, 0], "init wavefront")
        plt.colorbar(im, ax=ax[1, 0], orientation='horizontal')
        # 绘制保存的最优波前
    ...
    click.echo(f"RMS元启发式优化完成，最优RMS值: {min_rms:.4f} @ epoch {min_epoch}，使用算法: {algorithm}")