...
def zernike_radial(n, m, rho):
    ...
    for k in range((n - m_abs) // 2 + 1):
        ...
        kf = factorial_jit(k)
        af = factorial_jit((n + m_abs) // 2 - k)
        bf = factorial_jit((n - m_abs) // 2 - k)
        c = ((-1)**k * nf) / (kf * af * bf)
        R += c * rho**(n - 2*k)
    return R
...
# 非Numba版本的基本函数（用于兼容性和特殊情况）
def zernike_radial_py(n: int, m: int, rho: np.ndarray) -> np.ndarray:
    """
    计算Zernike径向多项式（Python版本）
...
def zernike_polynomial(n: int, m: int, rho: np.ndarray, 
    ...
    R = zernike_radial_py(n, np.abs(m), rho)
...
class ZernikeGenerator:
    """
    基于Numba优化的Zernike生成器
    
    使用Numba优化的函数进行高效计算，适用于需要性能优化的场景。
    """
    ...
    def __init__(self, zernike_indices, R, Theta, mask):
        ...
        参数:
            zernike_indices: Zernike模式的(n, m)列表
            R: 径向坐标网格
            Theta: 角坐标网格
            mask: 孔径掩膜
        """
