...
- ``ShapingObjective``: which shaping objective a search frame is scored with
  (seven families), the cross-objective ``m_*`` metric panel, the adaptive
  ``rms_pib`` term weighting and the in-ROI energy-loss safety guard.

All functions are pure (no hardware access; ``target_shape_roi`` logs a warning
only when it must shrink the target ROI to fit the frame). The module is a leaf
of ``ao_shaping.utils`` and must not import from ``algorithm`` / ``drivers`` /
``optimizer``. ``ShapingObjective`` therefore receives its
``ImageTargetFunc``-like bucket/radius helper *injected* by the caller instead
of importing the algorithm layer.
"""
...
# ---------------------------------------------------------------------------
# Objective / metric computations (CCD frame space)
# ---------------------------------------------------------------------------
# Target-shaped ROI builder and the beam-shaping objective/metric family used by
# the SLM-Zernike PIB optimizer (``optimizer.wfless.slm_zernike_pib``).
# ``target_shape_roi`` is the sibling of ``create_target_shape``: it turns a
# parametric target into a boolean ROI mask on a CCD frame. The metrics below
# operate on CCD frames and are exposure / laser-drift invariant (pure ratios or
# sum-normalised comparisons).
...
# Mirrors the ``TargetShape`` Literal kept in
# ``optimizer.wfless.slm_zernike_pib`` (as-is); ``target_shape_roi`` needs it
# for the ``cast`` below.
TargetShape = Literal[
    "gaussian",
    "circle",
...
# ---------------------------------------------------------------------------
# Shaping objective: adaptive-weight helpers + ``ShapingObjective``
# ---------------------------------------------------------------------------
...
#: Objectives the in-ROI energy-loss guard applies to (``radiu``/``avg_radiu``
#: have no target ROI to protect).
