...
def search_offset_by_defocus(
    ...
    for offset_y in range(-search_range, search_range + 1, search_step):
        for offset_x in range(-search_range, search_range + 1, search_step):
            ...
            click.echo(
                f"Offset ({offset_x:3d}, {offset_y:3d}): defocus={avg_defocus:.4f}±{std_defocus:.4f}"
            )
...
def optimize_defocus(
    ...
    defocus_values = np.arange(
        -defocus_range, defocus_range + defocus_step, defocus_step
    )
...
def search_offset_by_vortex(
    ...
    for offset_y in range(-search_range, search_range + 1, search_step):
        for offset_x in range(-search_range, search_range + 1, search_step):
            ...
            click.echo(
                f"Offset ({offset_x:3d}, {offset_y:3d}): intensity={avg_intensity:.2f}, metric={metric:.2f}"
            )
...
def run(
    ...
    with (
        ...
        if method == "defocus":
            ...
            if best_defocus != 0:
                click.echo(
                    f"或固定离焦: --shift-x {best_x} --shift-y {best_y} 并在优化器中设置初始离焦系数"
                )
        else:
