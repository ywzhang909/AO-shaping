...
def run(ctx: click.Context, params: AltVoltageRunnerParams) -> None:
    ...
    if params.port is None:
        try:
            last_octet = int(params.ip.split(".")[-1])
    ...
    # Parse channels
    if params.channel_str is None or params.channel_str.lower() == "all":
        channels = list(range(SINGLE_CHANNELS))
    else:
        try:
            channels = [int(c.strip()) for c in params.channel_str.split(",")]
    ...
    dur_desc = f"{params.alt_duration:.1f} 秒" if params.alt_duration > 0 else "持续运行 (Ctrl+C 停止)"
    click.echo("")
    click.echo("=" * 54)
    click.echo("  交替电压下发启动")
    click.echo("=" * 54)
    click.echo(f"  控制器:      {params.ip}:{port}")
    click.echo(f"  电压范围:    0V ↔ {params.alt_voltage:.1f}V")
    click.echo(f"  频率:        {params.alt_freq:.2f} Hz")
    click.echo(f"  周期:        {1.0 / params.alt_freq:.3f} 秒")
    click.echo(f"  通道:        {ch_desc}")
    click.echo(f"  持续时间:    {dur_desc}")
    if adc is not None:
        click.echo(f"  ADC:          {params.adc_device}/{params.adc_channel}, {params.adc_sample_rate} Hz, {params.adc_samples_per_read} smp/rd")
    click.echo("=" * 54)
    click.echo("")
