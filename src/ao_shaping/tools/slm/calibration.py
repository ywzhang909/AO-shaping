...
包含两个类:
  SLMCCDCalibrator   装配辅助 + 光束位置 + 几何标定 + 效果验证 (当前 CLI 唯一流程)
...
CLI (几何标定专用; LUT 标定请用 `slm-lut` 命令):
  python slm_calib_toolkit.py                          # 全流程 (装配->光束->几何->验证)
  python slm_calib_toolkit.py --skip-align --skip-beam # 只重新标定
  python slm_calib_toolkit.py --verify-only calib.npz  # 只验证已有标定
"""
