...
from ao_shaping.optimizer.wfless.pib import optimize_pib, optimize_pib_heuristic
...
@click.option("-r", "--r_bucket", default=0, help="半径桶大小 (default: 0，环围半径)。若设置为0，则根据功率半径自动调整。")
...
def run_spgd_pib(root_dir, load_file, cam_id, center, exposure_time_ms, epochs, r_bucket, 
...
@click.option("-s", "--cam_size", default=200, help="相机开窗大小 (default: 200*200)")
@click.option("-b", "--target_max_brightness", default=90, help="目标最大亮度值 (default: 90), 若为0则不自动调整曝光时间")
@click.option("--algorithm", default='PSO', type=click.Choice(['PSO', 'RUN', 'GA']), help="选择元启发式算法 (default: PSO)")
@click.option("--debug", is_flag=True, help="是否开启调试模式 (default: False)")
@click.option("--show", is_flag=True, help="显示远场光斑CCD图像和优化历史 (default: False)")
def run_heuristic_pib(root_dir, load_file, cam_id, center, exposure_time_ms, epochs, r_bucket, 
        delta, lr, weight_decay, shrink_iter, shrink_ratio, cam_size, target_max_brightness, 
        algorithm, debug, show):
    """使用元启发式算法优化轴向光束"""
    
    # 函数内部不再需要导入re，因为它已在文件顶部导入
    ...
    dm_unit_mask[0] = False
    # dm_unit_mask[38:] = False
    res_list = optimize_pib_heuristic(
        center=center,
        r_bucket=r_bucket,
        epochs=epochs,
        delta=delta,
    ...
    best_iter, (max_j_id, max_j) = res_list.get_best_iter()    
    if debug:
        save_dir = gen_date_dir(f'{root_dir}/heuristic')
        saved_file_name = gen_file_path_uuid(save_dir, 'pkl')
        res_df.to_pickle(saved_file_name, compression='zip')
    ...
    click.echo(f"使用{algorithm}算法的轴向光束优化完成，最优PIB值: {max_j:.4f} @ epoch {max_j_id}")
