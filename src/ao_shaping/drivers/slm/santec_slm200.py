...
class SantecSLM200:
    ...
    def __init__(
        ...
        try:
            ...
            raise SantecSLM200Error(
                f"无法导入SLM SDK (_slm_win): {e}. "
                "请确保已安装Santec SLM驱动程序。"
            )
    ...
    def open(self) -> None:
        ...
        if ret != SLM_OK:
            raise SantecSLM200Error(
                f"无法打开SLM #{self.slm_number}, 错误码: {ret}"
            )
    ...
    def set_wavelength(
        ...
        设置SLM的工作波长和对应的相位范围。此操作可能需要约40秒完成。
        通常只需在首次使用或更换波长时调用一次。
        ...
        logger.info(
            ...
            f"相位范围: 0~{phase_range*0.01:.2f}π, "
            f"2π对应灰度值: {self._max_grayscale_for_2pi}"
        )
    
    def get_wavelength_info(self) -> Tuple[int, float, int]:
        """获取当前波长设置信息
        
        Returns:
            Tuple of (wavelength_nm, phase_range_pi, max_grayscale_for_2pi)
    ...
    def create_phase_from_array(
        ...
        max_grayscale: Optional[int] = None
    ) -> np.ndarray:
        """将弧度相位值转换为SLM灰度值
