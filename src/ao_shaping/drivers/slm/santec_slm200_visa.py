...
class SantecSLM200Visa:
        ...
        try:
            if command == '*IDN?':
                ...
                if len(parts) >= 2:
                    ...
                    self._slm.set_wavelength(wavelength, self._phase_range)
                else:
                ...
                parts = command.split()
                if len(parts) >= 2:
                    phase_range = int(parts[1])
                    self._slm.set_wavelength(self._wavelength, phase_range)
                else:
                    raise VisaError("PHASE:RANGE 命令需要参数")
    ...
    def query(self, command: str) -> str:
        ...
        try:
            if command == '*IDN?':
                ...
                wl, _, _ = self._slm.get_wavelength_info()
                return str(wl)
            
            elif command == 'PHASE:RANGE?':
                _, pr, _ = self._slm.get_wavelength_info()
                return str(int(pr * 100))  # 转换为 0.01*pi 单位
    ...
    def set_wavelength(self, wavelength: int, phase_range: int = 200) -> None:
        """设置波长和相位范围
        
        Args:
            wavelength: 波长（nm）
            phase_range: 相位范围（0.01*pi）
        """
        self._slm.set_wavelength(wavelength, phase_range)
        self._wavelength = wavelength
        self._phase_range = phase_range
