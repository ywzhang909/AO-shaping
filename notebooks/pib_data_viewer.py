...
# %%
data = pd.read_pickle(
    "../data/wf-less/20260331_170725/cd3b8e90-bf3d-45a2-b8fd-a9b5556dff7f.pkl",
    compression="zip",
)
...
circle_sm = Circle(
    ...
    color="yellow",
    linewidth=2,
    label=f"Second Moment Radius: {radius_sm:.1f}",
)
circle_gauss = Circle(
    (w // 2, h // 2),
    radius_gauss,
    ...
    linewidth=2,
    linestyle="--",
    label=f"Gaussian Radius: {radius_gauss:.1f}",
)
...
# 绘制二阶矩半径圆
circle_sm_min = Circle(
    (w // 2, h // 2),
    min_radius,
    fill=False,
    color="yellow",
    linewidth=2,
    label=f"Second Moment Radius: {min_radius:.1f}",
)
...
anim = FuncAnimation(
    fig, update, frames=len(process_data), init_func=init, interval=100, blit=True
)
